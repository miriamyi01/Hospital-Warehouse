    ...
    while(ch=='S'){
		...
		while((fread(&info,sizeof(info),1,ptr))==1){
			if((strcmp(info.id,tar)<0) || (strcmp(info.id,tar)>0)){
				...
				tic.medi_num=a;
				tic.medi_iva=info.venta;
				tic.total=info.venta*a;

				fprintf(fp,"%s %s %s %d %.2f %.2f %d %d %d\n",tic.ticketno,tic.cnombre,tic.medir_nombre,tic.medi_num,tic.medi_iva,tic.total);
				fflush(stdin);

				fclose(fp);
				strcpy(s_r.medi_id,tar);
				strcpy(s_r.medir_nombre,info.medi_nombre);
				s_r.num=a;
				s_r.iva=info.venta;
				s_r.total=info.venta*a;

				/* Reporte de Venta */
				fprintf(ptrs_r,"%s %s %s %d %.2f %.2f %d %d %d\n",s_r.medi_id,s_r.medir_nombre,s_r.clien_nombre,s_r.num,s_r.iva,s_r.total);
				fflush(stdin);
				fclose(ptrs_r);

				/* Reporte de Compra */
				strcpy(pr_r.medi_id,tar);
				strcpy(pr_r.medir_nombre,info.medi_nombre);
				pr_r.num=a;
				pr_r.iva=info.venta;
				pr_r.unidad=info.unidad;
				fprintf(ptrpr_r,"%s %s %d %d %d %d %.2f %.2f  %.2f\n",pr_r.medi_id,pr_r.medir_nombre,pr_r.num,pr_r.unidad,pr_r.iva,pr_r.ganancia);
				fflush(stdin);
				fclose(ptrpr_r);
            }
		}
...
void ticket(){
        ...
        printf("%s",id);
        printf("\nNombre del Cliente: ");
        printf("\nNo   Producto       Cantidad          IVA         Total ");
        printf("\n--------------------------------------------------------");
