...
void medi_entrada(){
    ...
    ptrc_r=fopen("provereporte.dat","a");
    ch='S';
    while(ch=='S'){
        system("cls");
        printf("\t\t----------------------");
...
void medi_venta(){
    ...
    while(ch=='S'){
		...
		ptrg_r = fopen("comprareport.dat","a");
		system("cls");
		...
		while((fread(&info,sizeof(info),1,ptr))==1){
			if((strcmp(info.id,tar)<0) || (strcmp(info.id,tar)>0)){
				...
				/* Reporte de Venta */
				fprintf(ptrv_r,"%s %s %s %d %.2f %.2f %d %d %d\n",v_r.medi_id,v_r.medir_nombre,v_r.clien_nombre,v_r.num,v_r.sub,v_r.total,v_r.total,v_r.sDia,v_r.sMes,v_r.sAno);
                fflush(stdin);
                fclose(ptrv_r);
				...
				fprintf(ptrg_r,"%s %s %d %d %d %d %.2f %.2f %.2f\n",g_r.medi_id,g_r.medir_nombre,d,g_r.sMes,g_r.sAno,g_r.num,g_r.unidad,g_r.sub,g_r.ganancia);
				fflush(stdin);
				fclose(ptrg_r);
            }
		}
...
void ganancia_rpt(){
    ...
    printf("Presiona cualquier ENTER para regresar al MENU\n\n");
    printf("\n\t\t\tREPORTE DE GANANCIAS\n");
    printf("ID \tPRODUCTO \tFECHA      CANTIDAD \tPRECIO UNTARIO \tPRECIO VENTA \tSUB \tGANANCIA");
    printf("\n=================================================================================================================\n");
    while(fscanf(ptrg_r,"%s %s %d %d %d %d %f %f %f \n",g_r.medi_id,g_r.medir_nombre,&g_r.sDia,&g_r.sMes,&g_r.sAno,&g_r.num,&g_r.unidad,&g_r.sub,&g_r.ganancia)!=EOF){
		printf("%s",g_r.medi_id);
		printf("\t%s",g_r.medir_nombre);
		printf("\t%d-%d-%d",g_r.sDia,g_r.sMes,g_r.sAno);
		printf("\t%d",g_r.num);
...
void venta_rpt_diaria(){
    ...
    printf("Presiona cualquier tecla para regresar al MENU\n\n");
    printf("\n\t\t\tREPORTE DE VENTAS\n");
    printf("ID \tPRODUCTO \tNOMBRE DEL CLIENTE     CANTIDAD \tSUB \tTOTAL \tECHA");
    printf("\n=================================================================================================================\n");
    while(fscanf(ptrv_r,"%s %s %s %d %f %f %d %d %d\n",v_r.medi_id,v_r.medir_nombre,v_r.clien_nombre,&v_r.num,&v_r.sub,&v_r.total,&v_r.sDia,&v_r.sMes,&v_r.sAno)!=EOF){
		...
		printf("\t%.2f",v_r.sub);
		printf("\t%.2f",v_r.total);
		printf("\t%d-%d-%d",v_r.sDia,v_r.sMes,v_r.sAno);
...
void com_rpt_diaria(){
    ...
    printf("\n======================================================================\n");
    while(fscanf(ptrc_r,"%s %s %s %d %f %f %d %d %d\n",c_r.medi_id,c_r.medir_nombre,c_r.prove_nombre,&c_r.num,&c_r.sub,&c_r.total,&c_r.sDia,&c_r.sMes,&c_r.sAno)!=EOF){
        if(d==c_r.sDia &&m==c_r.sMes &&an==c_r.sAno){
            ...
            printf("\t%.2f",c_r.sub);
            printf("\t%.2f",c_r.total);
            printf("\t%d-%d-%d",c_r.sDia,c_r.sMes,c_r.sAno);
